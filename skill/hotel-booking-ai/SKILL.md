...
If `search_hotels` did not return `web_url`, omit the entire `👉 More hotels` paragraph. Never ask the user for `user_key` merely to populate this optional link. In Chinese, use this exact opening:
...
For hotel check-in/out times, instructions and mandatory at-property fees, use the selected hotel's `get_hotel_detail` tool result. Show unavailable fields as `酒店未提供` rather than guessing. In Chinese, use this exact final confirmation template (translate it for other languages while preserving all fields):
